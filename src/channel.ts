import { reqChzzk, exChzzk } from './tool';
...
async function search(keyword) {
  return new Promise(async (resolve, reject) => {
    let chSearch = await reqChzzk(
      'service/v1/search/channels?keyword=' + keyword + '&offset=0&size=13&withFirstChannelContent=false'
    );
    ...
    for (let o in chSearch) {
      ...
      chRes[Object.keys(chRes).length] = new chzzkChannel(
        tempChannel.channelId,
        tempChannel.channelName,
        tempChannel.channelDescription,
        tempChannel.followerCount,
        tempChannel.channelImageUrl,
        tempChannel.openLive
      );
    }
...
async function get(channelID) {
  return new Promise(async (resolve, reject) => {
    ...
    let chRes = new chzzkChannel(
      chGet.channelId,
      chGet.channelName,
      chGet.channelDescription,
      chGet.followerCount,
      chGet.channelImageUrl,
      chGet.openLive
    );
